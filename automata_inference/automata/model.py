...
class Automaton(ABC):
    ...
    def get_transitions(
        self, source: StateLike, target: StateLike, symbol: str | None = None
    ) -> set[Transition]:
...
class PGA(Automaton):
    ...
    def concat(self, other: PGA) -> PGA:
        ...
        new_transition_matrix = (
            ...
            | {
                ...
                for ((c1, source), (c2, target)) in set(
                    product(self.final, other.initial)
                )
            }
        )
        return PGA(
    ...
    def substitute(self, indeterminate: str, value: int) -> PGA:
        ...
        if value == 1:
            new_transition_matrix = new_transition_matrix | {
                Transition(
                    transition.source, transition.target, weight=transition.weight
                )
        ...
        return minimize(
            PGA(self.states, new_transition_matrix, self.initial, self.final)
        )
    ...
    def transition_substitution(self, indeterminate, other: PGA) -> PGA:
        ...
        """
        indet_trans = list(
            self.get_transitions_for_symbol(indeterminate)
        )  # Order matters here
        ...
        new_transition_matrix = {
            transition
            for transition in self.transition_matrix
            if transition not in indet_trans
        }
        ...
        for i, transition in enumerate(indet_trans):
            ...
            new_transition_matrix = new_transition_matrix | {
                Transition(IndexedState(state, i), transition.target, weight=c)
                for (c, state) in other.final
            }
    ...
    def decrement(self, indeterminate: str) -> PGA:
        ...
        first_indeterminate_transitions = {
            ...
            if transition.source.right != transition.target.right
        }
        ...
        updated_filtered = PGA(
            filtered.states, new_transition_matrix, filtered.initial, filtered.final
        )
        ...
        return (
            updated_filtered.weighted_union(subs_zero, 1, 1)
            if len(subs_zero.final) != 0
            else updated_filtered
        )
    ...
    def _construct_marginalized_transition_matrix(self, states: list[StateLike]):
        ...
        for transition in self.transition_matrix:
            pos_source, pos_target = states.index(transition.source), states.index(
                transition.target
            )
    ...
    def _construct_initial_weights_vector(
        self, states: list[StateLike]
    ) -> list[list[Rational]]:
        arr = [0] * len(states)
    ...
    def _construct_final_weights_vector(
        self, states: list[StateLike]
    ) -> list[list[Rational]]:
        arr = [0] * len(states)
    ...
    def normalize(self) -> PGA:
        ...
        normalization_coeff = Rational(
            probability_mass.denominator, probability_mass.numerator
        )
