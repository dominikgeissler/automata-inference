...
class StateLike: State | ProductState | IndexedState
...
class Automaton(ABC):
    ...
    def get_transitions(self, source: StateLike, target: StateLike) -> list[Transition]:
        ...
        Returns:
            list[Transition]: The list of transitions between the specified states.
        """
        return [transition for transition in self.transition_matrix if transition.source == source and transition.target == target]
    
    def get_transitions_for_symbol(self, symbol: str | None) -> list[Transition]:
        ...
        Returns:
            list[Transition]: The list of transitions for the specified symbol (or constant transitions if symbol is None).
        """
    ...
    def get_symbols(self) -> set[str]:
        ...
        """
        return {transition.symbol for transition in self.transition_matrix}
...
class DFA(Automaton):
    ...
    Args:
        states (set[State]): The set of states.
        transition_matrix (list[Transition]): The transition matrix. (Note: For DFA, the transition matrix is unweighted, so the weight attribute of Transition is not used.)
...
class PGA(Automaton):
    ...
    def concat(self, other: PGA) -> PGA:
        ...
        """
        new_transition_matrix = self.transition_matrix + other.transition_matrix + [
            Transition(source, target, weight=c1 * c2) for ((c1, source), (c2, target)) in set(product(self.final, other.initial))
        ]
    ...
    def substitute(self, indeterminate: str, value: int) -> PGA:
        ...
        new_transition_matrix = [
            transition for transition in self.transition_matrix if transition not in changed_transitions
        ]
    ...
    def filter(self, other: DFA) -> PGA:
            ...
            new_states = {ProductState(state1, state2) for state1 in self.states for state2 in other.states}

            new_transition_matrix = []

            for symbol in self.get_symbols().intersection(other.get_symbols()):
                self_transitions = self.get_transitions_for_symbol(symbol)
                other_transitions = other.get_transitions_for_symbol(symbol)
                
                # Add transitions if the symbols on them coincide (and muliply the weights)
                new_transition_matrix.extend(
                    Transition(
            ...
            new_initial = set((c, ProductState(state1,state2)) for (c, state1) in self.initial for state2 in other.initial)
            new_final = set((c, ProductState(state1,state2)) for (c, state1) in self.final for state2 in other.final)
            from automata_inference.automata.operations.minimization import minimize
    ...
    def transition_substitution(self, indeterminate, other: PGA) -> PGA:
            ...
            """
            indet_trans = self.get_transitions_for_symbol(indeterminate)
            ...
            new_transition_matrix = [
                transition for transition in self.transition_matrix if transition not in indet_trans
            ]
            
            # Connect each A_2 instance
            new_transition_matrix += [
                Transition(IndexedState(transition.source, i), IndexedState(transition.target, i), transition.symbol, transition.weight) for transition in other.transition_matrix for i in range(len(indet_trans))
            ]
            
            for i, transition in enumerate(indet_trans):
                # Add connections to A_2 instances
                new_transition_matrix += [
                    Transition(transition.source, IndexedState(state, i), weight=transition.weight * c) for (c, state) in other.initial
                ]    
    ...
    def decrement(self, indeterminate: str) -> PGA:
        ...
        first_indeterminate_transitions = [
            transition for transition in filtered.transition_matrix if transition.source.right != transition.target.right
        ]
        
        new_transition_matrix = [
            transition
            for transition in filtered.transition_matrix
            if transition not in first_indeterminate_transitions
        ]
