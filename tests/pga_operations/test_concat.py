...
def test_concat_same_var():
    ...
    concat = aut1.concat(aut2)
    assert expected.states == concat.states, f"States do not match, expected {expected.states}, got {concat.states}"
    assert expected.initial == concat.initial, f"Initial states do not match, expected {expected.initial}, got {concat.initial}"
    assert expected.final == concat.final, f"Final states do not match, expected {expected.final}, got {concat.final}"
    assert compare_dicts_with_unordered_lists(expected.transition_matrix, concat.transition_matrix), f"Transition matrices do not match, expected {expected.transition_matrix}, got {concat.transition_matrix}"
