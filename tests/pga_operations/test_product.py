...
def test_product_true():
    ...
    aut = create_pga(
        0,
        3,
        [(0, 1, "X", Rational(1, 2)), (0, 2, "Y", Rational(1, 2))],
        [(1, 0)],
        [(1, 1), (1, 2)],
    )
    ...
    expected = PGA(
        {
            ProductState(State(0, 0), State(1, 0)),
            ProductState(State(0, 1), State(1, 0)),
            ProductState(State(0, 2), State(1, 0)),
        },
        {
            Transition(
                ProductState(State(0, 0), State(1, 0)),
        ...
        {
            (1, ProductState(State(0, 1), State(1, 0))),
            (1, ProductState(State(0, 2), State(1, 0))),
        },
    )  # Nothing is filtered
...
# TODO add test with constant weights
...
def test_product_filter():
    ...
    # Models "x mod 2 = 1"
    dfa = DFA({t1, t2}, {Transition(t1, t2, "X"), Transition(t2, t1, "X")}, {t1}, {t2})
    ...
    expected = PGA(
        {p1, p2},
        {
            Transition(p1, p2, "X", Rational(1, 2)),
            Transition(p2, p1, "X", Rational(1, 2)),
        },
        {(1, p1)},
        {(Rational(1, 2), p2)},
    )
