...
def test_transition_substitution_changes_path():
    ...
    expected = PGA(
        {s0, s1, q1},
        {
            Transition(s0, q1, weight=Rational(1, 3)),
            Transition(q1, q1, "X", Rational(1, 2)),
            Transition(q1, s1, weight=Rational(1, 2)),
        },
        {(1, s0)},
        {(1, s1)},
    )
...
def test_transition_substitution_zero_pga():
    ...
    aut = create_pga(
        0, 1, [(0, 0, "X", Rational(1, 3))], [(1, 0)], [(Rational(2, 3), 0)]
    )
...
def test_transition_substitution_one_pga():
    ...
    aut = create_pga(
        0, 2, [(0, 1, "X", Rational(1, 3))], [(1, 0)], [(Rational(2, 3), 1)]
    )
...
def test_transition_substitution_loop():
    ...
    subs = create_pga(
        1, 1, [(0, 0, "Y", Rational(3, 4))], [(1, 0)], [(Rational(1, 4), 0)]
    )
    ...
    expected = PGA(
        {s, t},
        {
            Transition(s, t, weight=Rational(1, 3)),
            Transition(t, t, "Y", Rational(3, 4)),
            Transition(t, s, weight=Rational(1, 4)),
        },
        {(1, s)},
        {(Rational(2, 3), s)},
    )
...
def test_transition_substitution_multiple_transitions():
    ...
    aut = create_pga(
        0,
        3,
        [(0, 0, "X", Rational(1, 2)), (0, 1, "X", Rational(1, 2)), (1, 2, "Y", 1)],
        [(1, 0)],
        [(1, 2)],
    )
    subs = create_pga(
        1,
        2,
        [(0, 0, "X", Rational(1, 3)), (0, 1, "Z", 1)],
        [(1, 0)],
        [(Rational(2, 3), 1)],
    )
    s1, s2, s3 = State(0, 0), State(0, 1), State(0, 2)
    t11, t12, t21, t22 = (
        IndexedState(State(1, 0), 0),
        IndexedState(State(1, 1), 0),
        IndexedState(State(1, 0), 1),
